...
class TagLibCx extends TagLib {
    ...
    public function _volist($attr,$content) {
        ...
        $empty=explode('/', $empty);
        $empty="<div class='{$empty[1]}'>{$empty[0]}</div>";
			...
			if (C('MIN_MERGE_JS')){
				foreach ($array as $val){
					...
					case 'js':
						...
						$_SESSION['min_merge']['JS'][]=$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.js';
						$parseStr .='@MIN_AND_MERGE_JS'.count($_SESSION['min_merge']['JS']).'@';
						break;
					case 'css':
						//$parseStr .= '<link rel="stylesheet" type="text/css" href="'.$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.css" />';
						$_SESSION['min_merge']['CSS'][]=$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.css';
						$parseStr .='@MIN_AND_MERGE_CSS'.count($_SESSION['min_merge']['CSS']).'@';
						break;
					case 'php':
