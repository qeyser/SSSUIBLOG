...
class  ThinkTemplate {
    ...
    protected function merge($mmfiles,$tmplContent,$type='js'){
        $type=strtoupper($type);
        $mmfiles=array_unique($mmfiles);
        $merge_count=count($mmfiles);
        $mm_last = array_pop($mmfiles);
        $mmi=0;
        foreach ($mmfiles as $mmv) {
            if (substr($mmv, 0,1)=='/'){
                $mmv=substr($mmv,1);
            }
            if(is_file($mmv)){
                $mmarray[]=file_get_contents($mmv);
            }
            $mmi++;
            $tmplContent=str_replace('@MIN_AND_MERGE_'.$type.$mmi.'@','',$tmplContent);
        }
        if(is_file($mm_last)){
            $mmarray[]=file_get_contents($mm_last);
        }
        $mmstr=implode("\r\n",$mmarray);
        if (!empty($mmstr)){
            $filename=CACHE_PATH.time().'.'.$type;
            file_put_contents($filename,$mmstr);
            if ($type=='JS'){
                $tmplContent=str_replace('@MIN_AND_MERGE_'.$type.$merge_count.'@','<script type="text/javascript" src="'.$filename.'"></script>',$tmplContent);
            }else{
                $tmplContent=str_replace('@MIN_AND_MERGE_'.$type.$merge_count.'@','<link rel="stylesheet" type="text/css" href="'.$filename.'">',$tmplContent);
            }
        }else{
            $tmplContent=str_replace('@MIN_AND_MERGE_'.$type.$merge_count.'@','',$tmplContent);
        }
        return $tmplContent;
    }
    /**
    ...
    protected function compiler($tmplContent) {
        ...
        if (C('MIN_MERGE_JS')){
            $tmplContent=$this->merge($_SESSION['min_merge']['JS'],$tmplContent,'JS');
            $_SESSION['min_merge']['JS']='';
        }
        if (C('MIN_MERGE_CSS')){
            $tmplContent=$this->merge($_SESSION['min_merge']['CSS'],$tmplContent,'CSS');
            $_SESSION['min_merge']['CSS']='';
        }
        // 还原被替换的Literal标签
