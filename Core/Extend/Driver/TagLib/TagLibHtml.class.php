...
class TagLibHtml extends TagLib{
    ...
    /**
      ----------------------------------------------------------
     * editor标签解析 插入可视化编辑器
     * 格式： <html:editor id="editor" name="content" type="KE" style="" >{$vo.content}</html:editor>
      ----------------------------------------------------------
     * @access public
      ----------------------------------------------------------
    ...
    public function _editor($attr,$content) {
        ...
        switch(strtoupper($type)) {
            case 'KEMN':
                $parseStr   =	'<script charset="utf-8" src="__ROOT__/Public/Js/kindeditor-4.1.2/kindeditor-min.js"></script>
                                <textarea id="'.$id.'" name="'.$name.'" width="'.$width.'" height="'.$height.'" style="'.$style.'">'.$content.'</textarea>
                                <script type="text/javascript">
                                    var editor;
                                    KindEditor.ready(function(K) {
                                            editor = K.create("#'.$id.'",{
                                            fileManagerJson:"'.U('User/Article/fileManager').'",
                                            uploadJson:"'.U('User/Article/uploadJson').'",
            ...
            case 'KEFULL':
                ...
                break;
            case 'UE':
                $parseStr   =   '<script type="text/javascript" charset="utf-8" src="Public/ueditor/editor_config.js"></script>
				<script type="text/javascript" charset="utf-8" src="Public/ueditor/editor_all.js"></script>
				<textarea style="'.$style.'" name="'.$name.'" id="'.$id.'" width="'.$width.'" height="'.$height.'">'.$content.'</textarea>
				<script type="text/javascript">
				var editor = new UE.ui.Editor({
					imageUrl:"__URL__/imageUp"
				    ,imagePath:"../../../../"
				    ,scrawlUrl:"__URL__/scrawlUp"           //涂鸦上传地址
				    ,scrawlPath:"../../../../"
				    ,fileUrl:"__URL__/fileUp"               //附件上传提交地址
				    ,filePath: "../../../../"
				    ,catcherUrl:"__URL__/getRemoteImage"   //处理远程图片抓取的地址
				    ,catcherPath: "../../../../" 
				    ,imageManagerUrl:"__URL__/imageManager"       //图片在线管理的处理地址
				    ,imageManagerPath:"../../../../" 
				    ,getMovieUrl:"__URL__/getMovie"
				    ,toolbars:[['.C('editorconfig').']]
    ...
    public function _list($attr) {
        ...
        if(!empty($actionlist)) {//如果指定显示操作功能列
            $parseStr .= '<th >操作</th>';
        }
